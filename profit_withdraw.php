<?php
if (isset($_POST['submit']) && $_POST['taka'] > 0) {
    $category = field_protect($_POST['category']);
    $category_item = "Profit Withdraw";
    // $entry_date = field_protect($_POST['entry_date']);
    $taka = isset($_POST['taka']) ? (int)$_POST['taka'] : 0;
    $cash_paid = isset($_POST['cash_paid']) ? (int)$_POST['cash_paid'] : 0;
    $bank_paid = $_POST['bank_paid'];
    $member_id = $_POST['member_id'];
    $name = $_POST['name'];
    $ac_id = $_POST['ac_id'];
    $total = $cash_paid + $bank_paid ;
   
    if($total != $taka){
       echo "<div align='center' class='alert alert-warning' style='width:40%; font-size:20px; margin:0 auto;'>Sorry! Amount Not Match!! Try Again.</div>";
       exit();
    }
   
    $comment = field_protect($_POST['comment']);
 
     //New Transection Create

    $transections_add = array(
        "created_by" =>  $uid,
        "created_at" =>  $datetime,
        "created_ip" => $ip,
        "entry_date" => $today,
        "member_id" => $member_id,
        "category" => $category,
        "details" => $category_item,
        "comment" => $comment,
        "cash_out" => $taka,
        "status" => "Paid",
    );
    
        $transections_submit = insert_data($conn, $transections_add, "transections");
        $submited_id = $transections_submit['last_id'];
        
    if($submited_id){
     
     // Profit Decrease
    $profit_decrease = array(
        "transaction_id" => $submited_id,
        "entry_date" => $today,
        "member_id" => $member_id,
        "name" => $name,
        "ac_id" => $ac_id,
        "category" => $category,
        "comment" => $comment,
        "decrease" => $taka
                                                           
      
      
    );
    
        $profit_submit = insert_data($conn, $profit_decrease, "accounts_profit");
    $profit_id = $profit_submit['last_id'];
  
       // default
    $cash_id = 0; 
    $bank_id = 0;
    
    
    // Cash Decrease
     if($cash_paid > 0){
    $cash_decrease = array(
        "transection_id" => $submited_id,
        "entry_date" => $today,
        "category" => $category,
        "title" => $category_item,
        "decrease" => $cash_paid
        
    );
    
        $cash_submit = insert_data($conn, $cash_decrease, "cash");
    $cash_id = $cash_submit['last_id'];
     }
    //Bank Decreas
    if($bank_paid > 0){
        $bank_decrease = array(
        "transection_id" => $submited_id,
        "entry_date" => $today,
        "category" => $category,
        "title" => $category_item,
        "decrease" => $bank_paid
        
    );
    
        $bank_submit = insert_data($conn, $bank_decrease, "bank");
    $bank_id = $bank_submit['last_id'];
    }
    
    if(isset($profit_id, $cash_id, $bank_id)){
        echo "<div align='center' class='alert alert-success' style='width:40%; font-size:20px; margin:0 auto;'>$category Profit Withdraw Successfully</div>";
         }
      }
   }
    ?>
 <h2> Profit Withdraw</h2>
 <form method="post" action="">
         <div class="row mb-3">
             <div class="col-4">
                <label for="category Item" class="form-label">Category Item</label>
                <select class="form-control" name="category" id="category" required>
                    <option value="">-- SELECT --</option>
        <option value="Savings">Savings</option>
        <option value="Share">Share</option>
                   
           <?php $option_collection = option_collect("accounts_profit", "");
                        echo $option_collection['options'];
                       // $category = $option_collection['category'];
                        ?>
                    </select>
                    
                    
            </div>
            <div class="col-4">
                <label for="ac_id" class="form-label">Account No</label>
                <input type="text" name="account_no" id="account_no" class="form-control"  required>
                <input type="hidden" name="ac_id" id="ac_id" class="form-control"  required>
            </div>
            <div class="col-4">
                <label for="name" class="form-label">Member ID</label>
                <input type="text" name="member_id" id="member_id" class="form-control"  required>
            </div>
             <div class="col-4">
                <label for="name" class="form-label">Name</label>
                <input type="text" name="name" id="name" class="form-control"  required>
            </div>
    
            <div class="col-4">
                <label for="taka" class="form-label">Total Taka</label>
                <input type="number" name="taka" id="taka" class="form-control"  required>
            </div>
             <div class="col-4">
                <label for="taka" class="form-label">Paid BY Cash</label>
                <input type="number" name="cash_paid" class="form-control" value="0">
            </div>
            <div class="col-4">
                <label for="taka" class="form-label">Paid By Bank</label>
                <input type="number" name="bank_paid" class="form-control" value="0">
            </div>
             <div class="col-4">
                <label for="details" class="form-label">Comment</label>
                <input type="text" name="comment" id="comment" class="form-control">
            </div>
            
      </div>

       

        <button type="submit" name="submit" class="btn btn-primary">Submit</button>
</form>


 <script>
$("#account_no").autocomplete({
    source: function(request, response) {
        var category = $("#category").val();

        if (!category) {
            alert("Please select a Category first.");
            return;
        }

        $.ajax({
            url: "controllers/index.php",
            type: "POST",
            dataType: "json",
            data: {
                account_no: request.term,
                account_type: category
            },
            success: function(data) {
                // Pass the JSON data to autocomplete
                response(data);

                // Debug in console
                console.log("Autocomplete data:", data);
            },
            error: function(xhr, status, error) {
                console.error("Autocomplete error:", error);
            }
        });
    },
    select: function(event, ui) {
        // When an item is selected from the list
        $("#account_no").val(ui.item.value);        // Account number
        $("#name").val(ui.item.data.account_title); // Member name
        $("#member_id").val(ui.item.data.member_id); // Member id
        $("#ac_id").val(ui.item.data.id);           // Hidden ID
        $("#taka").val(ui.item.profit_balance); 
        $("#category").val(ui.item.data.account_type);  // Account type/category
        
        return false; // Prevent default
    },
    minLength: 1 // Start suggesting after 1 character
});
</script>

